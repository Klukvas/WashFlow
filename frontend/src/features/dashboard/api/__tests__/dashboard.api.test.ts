...
describe('dashboard.api', () => {
  ...
  it('fetchPopularServices calls GET /analytics/services', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({
      data: { data: [{ name: 'Wash', count: 10, revenue: 1000 }] },
    });
  ...
  it('fetchKpiData calls GET /analytics/kpi', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: { data: mockKpiData } });
  ...
  it('fetchLiveOperations calls GET /analytics/live', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: { data: mockLiveOps } });
    const result = await fetchLiveOperations({ branchId: 'b1' });
    expect(apiClient.get).toHaveBeenCalledWith('/analytics/live', {
      params: { branchId: 'b1' },
    });
  ...
  it('fetchBranchPerformance calls GET /analytics/branches', async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith('/analytics/branches', {
      params: undefined,
    });
    expect(result).toEqual([]);
  });
  ...
  it('fetchEmployeePerformance calls GET /analytics/employees', async () => {
    ...
    expect(apiClient.get).toHaveBeenCalledWith('/analytics/employees', {
      params: undefined,
    });
    expect(result).toEqual([]);
  });
  ...
  it('fetchAlerts calls GET /analytics/alerts', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: { data: [mockAlert] } });
    const result = await fetchAlerts();
    expect(apiClient.get).toHaveBeenCalledWith('/analytics/alerts', {
      params: undefined,
    });
    expect(result).toHaveLength(1);
  });
  ...
  it('fetchOnlineBookingStats calls GET /analytics/online-booking', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({
      data: { data: mockBookingStats },
    });
    const result = await fetchOnlineBookingStats();
    expect(apiClient.get).toHaveBeenCalledWith('/analytics/online-booking', {
      params: undefined,
    });
