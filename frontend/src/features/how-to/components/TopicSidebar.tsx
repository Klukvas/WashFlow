  ...
  return (
    <>
      {items.map((topic) => (
        <NavLink
          key={topic.slug}
          to={ROUTES.HOW_TO_TOPIC.replace(':topicSlug', topic.slug)}
          className={({ isActive }) =>
            cn(
        ...
        >
          <topic.icon aria-hidden="true" className="h-4 w-4 shrink-0" />
          <span className="truncate">{t(`topics.${topic.slug}.title`)}</span>
        </NavLink>
      ))}
    </>
  );
...
export function TopicSidebar({ showLabels = false }: { showLabels?: boolean }) {
  ...
  return (
    <nav aria-label={t('meta.topicNav')} className="flex flex-col gap-1">
      {showLabels && (
